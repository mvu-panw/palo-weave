...
- `techdocs.html` and `api-docs.html` (under "Technical Resources") each embed a single doc — placeholders, not yet wired to a real doc.
- `github.html` and `slide-decks-spreadsheets.html` (under "Technical Resources") are `.resource-grid`/`.resource-card` link lists (title + description + external link) rather than embeds — `github.html` for repo links, `slide-decks-spreadsheets.html` for Google Drive links. Both start empty; an HTML comment in each shows the card markup to add.
